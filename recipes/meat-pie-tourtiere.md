---
title: "Meat Pie (Tourtiere)"
description: "A delicious main course made with ground pork, baking potato. Simple to prepare and serves 8."
author: "Maggie Rogers"
prep_time: PT0M
cook_time: PT0M
servings: 8
calories: 485
recipe_category: Main Course
keywords:
  - course
  - main
  - meat
  - pie
  - tourtiere
recipe_ingredients:
  - "Pork"
  - "Baking Potato"
  - "Onion"
  - "Salt"
  - "Black Pepper"
  - "Cinnamon"
  - "Allspice"
  - "Water"
  - "Recipe Pastry A Double Crust Deep Dish Pie"
  - "Egg"
  - "Paprika"
allergies:
  - Eggs
flavors:
  - Sweet
  - Savory
tools:
  - Oven
  - Skillet
  - Cutting Board
  - Pie Dish
skill_level: Easy
source_url: "https://www.allrecipes.com/recipe/12343/meat-pie-tourtiere/"
---

## Ingredients

- 1 ½ pounds ground pork
- 1 large baking potato
- 1 large onion, minced
- ½ teaspoon salt
- ½ teaspoon ground black pepper
- ½ teaspoon ground cinnamon
- ¼ teaspoon ground cloves
- 1 dash ground allspice
- ½ cup water
- 1 recipe pastry for a 9 inch double crust deep dish pie
- 1  egg
- ¼ teaspoon paprika

## Instructions

1. Bake the potato until done, 30 - 45 minutes in a preheated 400 degrees F (205 degrees C) oven. Peel and mash the potato.
2. Place the potato, ground pork, onion, spices and water in a large frying pan and simmer until very thick, for about one hour.
3. Meanwhile, prepare your pastry.
4. Line a deep-dish pie plate with pastry. Spoon in filling, spreading evenly. Cover with top crust.
5. Brush with beaten egg and sprinkle with paprika, if desired. Cut steam vent. Bake for 50 minutes at 350 degrees F (175 degrees C). If edges brown too fast, cover with a strip of foil. Serve warm.

## Frequently Asked Questions

### How difficult is Meat Pie (Tourtiere) to make?

This recipe is rated easy difficulty.

### How many servings does this Meat Pie (Tourtiere) recipe make?

This recipe yields 8 servings. You can adjust the serving size using the servings slider above the ingredient list.

### Can I make Meat Pie (Tourtiere) ahead of time? How should I store leftovers?

Store leftovers in an airtight container in the refrigerator for up to 3-4 days.

### How many calories are in Meat Pie (Tourtiere)?

Each serving contains approximately 485 calories. The recipe serves 8.

### What temperature should I use for Meat Pie (Tourtiere)?

Follow the temperature specified in the recipe instructions. Make sure to preheat your oven fully.
