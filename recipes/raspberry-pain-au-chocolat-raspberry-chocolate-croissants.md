---
title: "Raspberry Pain au Chocolat (Raspberry Chocolate Croissants)"
description: "This easy-level bread features puff pastry, chocolate hazelnut spread and is ready in about 38 minutes. Serves 18."
author: "Allison"
prep_time: PT20M
cook_time: PT18M
servings: 18
calories: 189
recipe_category: Bread
keywords:
  - bread
  - chocolat
  - chocolate
  - croissants
  - pain
  - raspberry
recipe_ingredients:
  - "Puff Pastry"
  - "Chocolate Hazelnut Spread"
  - "All Fruit Raspberry Jam"
  - "Egg"
  - "Confectioners' Sugar Dusting"
allergies:
  - Eggs
  - Tree Nuts
flavors:
  - Sweet
tools:
  - Oven
  - Baking Sheet
  - Cutting Board
skill_level: Easy
source_url: "https://www.allrecipes.com/recipe/84393/raspberry-pain-au-chocolat-raspberry-chocolate-croissants/"
---

## Ingredients

- 1 (17.25 ounce) package frozen puff pastry, thawed
- 6 tablespoons chocolate hazelnut spread
- 3 tablespoons all fruit raspberry jam
- 1  egg, beaten
- ¼ cup confectioners' sugar for dusting  (Optional)

## Instructions

1. Preheat the oven to 400 degrees F (200 degrees C).
2. Unfold the puff pastry sheets on a lightly floured surface. Cut each one into thirds along the fold lines. Roll each strip of dough out to about 16 inches long, and cut strips into thirds.
3. Spread some of the hazelnut spread onto one half of each rectangle to within 1/2 inch of the edge. Spread about 1/2 teaspoon of the raspberry jam over the hazelnut spread. Brush the edges with egg, and fold over the other side to enclose the filling. Press the edges firmly to seal. Repeat with the remaining rectangles. Use a sharp knife to cut 5 slits across the top of each pastry, and place them on baking sheets, spacing 2 inches apart.
4. Bake for 18 minutes in the preheated oven, or until golden brown. Cool on racks. Dust with confectioners' sugar when cooled if desired.

## Frequently Asked Questions

### How long does it take to make Raspberry Pain au Chocolat (Raspberry Chocolate Croissants)?

This recipe takes approximately 38 minutes from start to finish — 20 minutes of prep and 18 minutes of cooking. It is rated as easy difficulty.

### How many servings does this Raspberry Pain au Chocolat (Raspberry Chocolate Croissants) recipe make?

This recipe yields 18 servings. You can adjust the serving size using the servings slider above the ingredient list.

### Can I make Raspberry Pain au Chocolat (Raspberry Chocolate Croissants) ahead of time? How should I store leftovers?

Store this bread at room temperature in a bread box or sealed bag for 2-3 days.

### How many calories are in Raspberry Pain au Chocolat (Raspberry Chocolate Croissants)?

Each serving contains approximately 189 calories. The recipe serves 18.

### What temperature should I use for Raspberry Pain au Chocolat (Raspberry Chocolate Croissants)?

Follow the temperature specified in the recipe instructions. Make sure to preheat your oven fully.
