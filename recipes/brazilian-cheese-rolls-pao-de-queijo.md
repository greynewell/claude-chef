---
title: "Brazilian Cheese Rolls (Pao de Queijo)"
description: "Ready in about 45 minutes, this bread uses tapioca starch, salt. Serves 14 at 199 calories each."
author: "Eileen Lee"
prep_time: PT20M
cook_time: PT25M
servings: 14
calories: 199
recipe_category: Bread
keywords:
  - brazilian
  - bread
  - cheese
  - pao
  - queijo
  - rolls
recipe_ingredients:
  - "Tapioca Starch"
  - "Salt"
  - "Vegetable Oil"
  - "Water"
  - "Milk"
  - "Egg"
  - "Parmesan Cheese"
allergies:
  - Dairy
  - Eggs
flavors:
  - Umami
tools:
  - Oven
  - Mixing Bowl
  - Baking Sheet
skill_level: Easy
source_url: "https://www.allrecipes.com/recipe/221430/brazilian-cheese-rolls-pao-de-queijo/"
---

## Ingredients

- 2 cups tapioca starch
- 1 teaspoon salt  (Optional)
- ½ cup vegetable oil
- ⅓ cup water
- ⅓ cup milk
- 2  eggs
- 6 ounces shredded Parmesan cheese

## Instructions

1. Preheat oven to 375 degrees F (190 degrees C). Lightly grease a baking sheet
2. Place tapioca starch and salt in a large bowl.
3. Bring vegetable oil, water, and milk to a boil over medium heat until a white foam appears. Pour milk mixture over tapioca starch and stir until well mixed; allow dough to rest for 15 minutes. Mix eggs and Parmesan cheese into dough. Shape dough into 1 1/2-inch balls and place on prepared baking sheet.
4. Bake in preheated oven until rolls are browned, 15 to 20 minutes.

## Frequently Asked Questions

### How long does it take to make Brazilian Cheese Rolls (Pao de Queijo)?

This recipe takes approximately 45 minutes from start to finish — 20 minutes of prep and 25 minutes of cooking. It is rated as easy difficulty.

### How many servings does this Brazilian Cheese Rolls (Pao de Queijo) recipe make?

This recipe yields 14 servings. You can adjust the serving size using the servings slider above the ingredient list.

### Can I make Brazilian Cheese Rolls (Pao de Queijo) ahead of time? How should I store leftovers?

Store this bread at room temperature in a bread box or sealed bag for 2-3 days.

### How many calories are in Brazilian Cheese Rolls (Pao de Queijo)?

Each serving contains approximately 199 calories. The recipe serves 14.

### What temperature should I use for Brazilian Cheese Rolls (Pao de Queijo)?

Follow the temperature specified in the recipe instructions. Make sure to preheat your oven fully.
